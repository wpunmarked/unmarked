<?php
/**
* Credit: https://github.com/BenSibley/ignite/blob/master/inc/breadcrumbs.php
*/

if ( ! function_exists( 'unmarked_breadcrumbs' ) ) {
	function unmarked_breadcrumbs( $args = array() ) {
		...
		if ( get_theme_mod( 'unmarked_show_breadcrumbs_setting' ) == 'no' ) {
			return;
		}
		...
		$args      = apply_filters( 'unmarked_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
		...
		if ( is_singular( 'post' ) ) {
			...
			$html .= '<span>' . __( 'Error 404', 'unmarked' ) . '</span>';
		} elseif ( is_home() ) {
			$html .= '<span>' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
		}

		$html .= '</div>';
		$html = apply_filters( 'unmarked_breadcrumbs_filter', $html );
